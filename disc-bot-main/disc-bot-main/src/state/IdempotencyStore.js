...
class IdempotencyStore {
    constructor() {
        ...
        this.cleanupOldPayments(30); // Purge records older than 30 days
    }
    ...
    /**
     * Clean up old payment records to prevent JSON bloat
     * @param {number} days - Max age in days
     */
    cleanupOldPayments(days = 30) {
        const threshold = Date.now() - (days * 24 * 60 * 60 * 1000);
        let purged = 0;

        for (const [id, payment] of Object.entries(this._payments)) {
            const createdAt = new Date(payment.createdAt).getTime();
            // Only purge if it's confirmed or failed. Never purge PENDING/BROADCAST.
            if ((payment.state === PaymentState.CONFIRMED || payment.state === PaymentState.FAILED) && createdAt < threshold) {
        ...
        if (purged > 0) {
            logger.info(`🧹 Idempotency cleanup: Purged ${purged} records older than ${days} days.`);
            this._save();
        }
    }
