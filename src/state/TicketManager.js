...
class TicketManager {
    ...
    createTicket(channelId, data) {
        ...
        // Update user index
        if (data.opponentId) {
            this.userIndex.set(data.opponentId, channelId);
    ...
    cleanupOldTickets(maxAgeMs = 24 * 60 * 60 * 1000) {
        const now = Date.now();
        // Use Array.from to safely iterate while modifying
        for (const [id, t] of Array.from(this.tickets.entries())) {
            if (t.isComplete() && (now - t.updatedAt) > maxAgeMs) {
                this.removeTicket(id); // Use removeTicket to clean up userIndex too
            }
        }
    }
    ...
    fromJSON(data) {
        ...
        for (const d of data) {
            ...
            if (t.data.opponentId && !t.isComplete()) {
                this.userIndex.set(t.data.opponentId, t.channelId);
